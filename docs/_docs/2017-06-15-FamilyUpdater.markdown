...
| 3 - Save Nested | See [here](#SaveNested) Saves out all nested families |
| 4 - Cancel | Cancels any user input and returns to Revit. |
| 5 – Process Families (x) | Will start to process all highlighted families. The number of families to be processed is shown in brackets. After completion a ‘Finished’ Message will be displayed. |
...
| Update | Action |
|---------|------------|
| Save Nested <a id="SaveNested"></a> | Will save out all nested families to a specified location. If multiple families with the same name are found, a counter will appended to the file name in format '[[x]]' |
