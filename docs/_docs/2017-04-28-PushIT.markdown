...
Rather than colouring the column families itself, the filter should target the nested and shared Generic Model family instead. This allows to colour the background of a room and leaves items placed in a room, e.g. beds, white and not effected by the filter.
...
*NOTE: it is STRONGLY advised never to change the unique ID of an SOA item once chosen, nor re-use an ID for another SOA item. Doing so will destroy the integrity of the SOA / model data comparison.*
...
![Export to tab-delimited txt file]({{ site.baseurl }}\assets\Pushit\PushIt-SOA-txt-File-Export.png){:class="img-responsive"}{: height="389px" width="593px"}
...
![Advanced sample SOA Excel file for Push It]({{ site.baseurl }}\assets\Pushit\PushIt-SOA-Excel-File-Sample-Advanced.png){:class="img-responsive"}{: height="320px" width="790px"}
...
| --- | ------------------------------------------ | ------ |
| 1 | ![Audit_PushITMain]({{ site.baseurl }}\assets\Pushit\Audit_PushITMain.png){:class="img-responsive"}{: height="133px" width="333px"} | In the Push iT interface press the <Audit> button as highlighted below. |
| 2 | ![Audit_PushITSaveTextFile]({{ site.baseurl }}\assets\Pushit\Audit_PushITSaveTextFile.png){:class="img-responsive"}{: height="133px" width="302px"} | Push it will analyse the model and will offer to save a text file if it found any inconsistencies |
| 3 | ![Audit_PushITSaveTextFileAsDialogue]({{ site.baseurl }}\assets\Pushit\Audit_PushITSaveTextFileAsDialogue.png){:class="img-responsive"}{: height="253px" width="426px"} | Select a location where to save the audit text file to. |
| 4 | ![Audit_ExcelOpenTextFile]({{ site.baseurl }}\assets\Pushit\Audit_ExcelOpenTextFile.png){:class="img-responsive"}{: height="260px" width="426px"} | In Excel open the text file via File Open Dialogue. (Make sure the file filter is to 'All files' or 'Text Files') |
| 5 | ![Audit_ExcelImportDelimited]({{ site.baseurl }}\assets\Pushit\Audit_ExcelImportDelimited.png){:class="img-responsive"}{: height="289px" width="386px"} | When opening a text file, Excel will want to know some bits of information about it. In the first window select 'Delimited' |
| 6 | ![Audit_ExcelImportDelimitedTab]({{ site.baseurl }}\assets\Pushit\Audit_ExcelImportDelimitedTab.png){:class="img-responsive"}{: height="289px" width="386px"} | In the following window select the delimeter type : 'Tab' and press 'Finish' |
| 7 | ![Audit_Excel_ApplyFilter]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter.png){:class="img-responsive"}{: height="274px" width="426px"} | Once Excel has imported the file press the little triangle top left corner to select all cells. With all cells selected apply a filter (under 'Sort & Filter') |
| - |  | The following filters are applied in no particular order |
| 8 | ![Audit_Excel_ApplyFilter_DesignOptions]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_DesignOptions.png){:class="img-responsive"}{: height="390px" width="481px"} | In the Design Option column click the filter icon and deselect all. Then select any option with the word '(primary)' at the end and 'Main Model' if present. This will remove all rooms from the list which are not in the primary desing option of a set or in the main model. |
| 9 | ![Audit_Excel_ApplyFilter_Families]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_Families.png){:class="img-responsive"}{: height="313px" width="438px"} | In the Family column click the filter icomn and remove any families with 'Lift', 'Stair' or 'Riser' in the name. Data in those families is most likely manually entered. |
| 10 | ![Audit_Excel_ApplyFilter_Department]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_Department.png){:class="img-responsive"}{: height="366px" width="350px"} | In the SP_Department column click the filter button and remove any unwanted department.i.e. 'Plant' |
| - | | The above filter ensure that any rooms left are of interest to us |
| 11 | ![Audit_Excel_ApplyFilter_RoomNumberSet]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_RoomNumberSet.png){:class="img-responsive"}{: height="312px" width="418px"} | In the SP_Room Number column click the filter icon and uncheck 'blank' option. Any rooms now left visible have a room number assigned but the data in the SP_ fields .i.e. name,  department or briefed area does not match what is currently in SOA |
| 12 | ![Audit_Excel_ApplyFilter_RoomNumberBlank]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_RoomNumberBlank.png){:class="img-responsive"}{: height="306px" width="393px"} | In the SP_Room Number column click the filter icon and uncheck everything *but* the 'blank' option. Any rooms now left visible have no room number assigned because they are most liekly not in the SOA yet and need to be added. |

To select any rooms highlighted in the above report:

- Copy the element ID, which is in the first column of the audit report
- use Revit's 'Select By Id' function to find the room.


## THINGS STILL TO BE COVERED:

- Any gotchas / limitations
...
- Remember last filter settings used (per user)
- DataGridView is too slow for large amount of rows when filtering the SOA list. Look at a better performing GUI component (WPF?) or ways at speeding up the filtering process.
- Link file support
- Database backend
    - Pros:
        - Single source of Truth - ensures everyone is working with up-to-date data.
        - Data integrity.
        - Web-accessible (if used in conjunction with a developed Web frontend)
    - Cons:
        - Significant development / reworking of source code required:
            - Database setup
            - Database Schemas
            - Database Permissions
            - Code interfacing logic
            - User Interface logic (Web frontend?)
...
![Newly Created room Family naming]({{ site.baseurl }}\assets\Pushit\GetARoom-Room-Family-Naming.png){:class="img-responsive"}{: height="124px" width="410px"}
...
### Changing shapes of custom rooms

To do this simply select the existing room family to be replaced and a filled region that describes the new shape.

![ReplaceCustomRooms]({{ site.baseurl }}\assets\Pushit\GetARoom_Replace.jpg){:class="img-responsive"}{: height="610px" width="529px"}

Note that because this operation transfers SOA information you need to have the Push It tool running and loaded SOA prior to using Get A Room. Also note that a new tag will need to be placed manually on the newly created family if there was one previously.

### Room for improvement

- Settings dialogs

## Converting Push It elements to Revit rooms

Once the layout is developed far enough and major changes to it are unlikely, Push It elements will need to be converted to native Revit elements of types: walls, doors and rooms.

Quite often this might be in form of a completely new model containing native Revit walls, doors and rooms.

### Walls and Doors

Walls and doors are created by tracing manually the current Push It layout using Revit native walls and doors. Depending on the size of the project this may take anything from a day to approximately a week.
Whilst this appears to be a step back at first, having this manual process offers up the opportunity to check, for instance, whether minimum width of corridors has been achieved or walls in general are set out to round dimensions of the nearest grid.

### Rooms

Native Revit rooms can be placed with the help of a Python script.
A sample script for Revit 2017 can be downloaded [here]({{ site.baseurl }}\assets\Pushit\place_rooms_for_push_it_2018.py). Please note that the script will need to be amended to suit your project requirements and / or version of Revit.

The Python script will place a room in the model at the centroid of every Push It element instance in the same Revit model and transfers, in this specific sample case, the SP_Room_Function_Number parameter value from the Push It element to the Revit room.

#### Rooms - Prep work

In order for the above script to work properly, the following conditions need to be met:

* All Push It elements will need to report a placement level.
* The shared parameter SP_Room_Function_Number needs to be attached to columns (Push It elements) and rooms.
* Any design options containing Push It elements should be removed from the model.

Note: Rooms will be created in the phase of the currently active view.

#### Rooms - Script errors

Since not all Push It elements are actually room bounding, ie. any spaces without walls are not, the following errors may occur as the script runs and places rooms in the model:

* 'Room is not in a properly enclosed region'
* 'Multiple Rooms are in the same enclosed region'

This can be addressed by placing room separation lines or walls in the new model as required.

There is also the slim chance that a room is actually placed by the script outside the bounds of the Push It element. This happens when the centroid of the Push It element is actually outside the perimeter of the element ( picture a u-shape).