...
* Specialty Rooms (Revit Category Column)
	...
	* Bed Rooms
		...
		* SP-Room-Bed-CutOut_Ens_Square_10
...
|---|---|---|
| 1 | ![Audit_PushITMain]({{ site.baseurl }}\assets\Pushit\Audit_PushITMain.png){:class="img-responsive"}{: height="600px" width="1000px"} | In the Push iT interface press the <Audit> button as highlighted below. |
...
| 3 | ![Audit_PushITSaveTextFileAsDialogue]({{ site.baseurl }}\assets\Pushit\Audit_PushITSaveTextFileAsDialogue.png){:class="img-responsive"}{: height="760px" width="1280px"} | Select a location where to save the audit text file to. |
| 4 | ![Audit_ExcelOpenTextFile]({{ site.baseurl }}\assets\Pushit\Audit_ExcelOpenTextFile.png){:class="img-responsive"}{: height="680px" width="1280px"} | In Excel open the text file via File Open Dialogue. (Make sure the file filter is to 'All files' or 'Text Files') |
...
| 7 | ![Audit_Excel_ApplyFilter]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter.png){:class="img-responsive"}{: height="823px" width="1279px"} | Once Excel has imported the file press the little triangle top left corner to select all cells. With all cells selected apply a filter (under 'Sort & Filter') |
| - |  | The following filters are applied in no particular order |
| 8 | ![Audit_Excel_ApplyFilter_DesignOptions]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_DesignOptions.png){:class="img-responsive"}{: height="780px" width="962px"} | In the Design Option column click the filter icon and deselect all. Then select any option with the word '(primary)' at the end and 'Main Model' if present. This will remove all rooms from the list which are not in the primary desing option of a set or in the main model. |
| 9 | ![Audit_Excel_ApplyFilter_Families]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_Families.png){:class="img-responsive"}{: height="626px" width="879px"} | In the Family column click the filter icomn and remove any families with 'Lift', 'Stair' or 'Riser' in the name. Data in those families is most likely manually entered. |
| 10 | ![Audit_Excel_ApplyFilter_Department]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_Department.png){:class="img-responsive"}{: height="732px" width="704px"} | In the SP_Department column click the filter button and remove any unwanted department.i.e. 'Plant' |
| - | | The above filter ensure that any rooms left are of interest to us |
| 11 | ![Audit_Excel_ApplyFilter_RoomNumberSet]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_RoomNumberSet.png){:class="img-responsive"}{: height="624px" width="836px"} | In the SP_Room Number column click the filter icon and uncheck 'blank' option. Any rooms now left visible have a room number assigned but the data in the SP_ fields .i.e. name,  department or briefed area does not match what is currently in SOA |
| 12 | ![Audit_Excel_ApplyFilter_RoomNumberBlank]({{ site.baseurl }}\assets\Pushit\Audit_Excel_ApplyFilter_RoomNumberBlank.png){:class="img-responsive"}{: height="612px" width="786px"} | In the SP_Room Number column click the filter icon and uncheck everything *but* the 'blank' option. Any rooms now left visible have no room number assigned because they are most liekly not in the SOA yet and need to be added. |
