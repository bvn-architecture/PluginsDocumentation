...
| SP_Functional Group | Colour all rooms of the same functional grouping |
...
If no filter is applied, a green colour room background is shown. Which in turn means, assuming that green means on area, one requires only two filters in total to show rooms which are under area and rooms which are above area with different colour backgrounds. Standard colours are red for under and blue for above area.
...
| 2d5fcb72-ae86-443e-b217-e08deb07d046 | Subdepartment(deprecated)  | TEXT     | Replaced by SP_Department parameter. Still in use in 'Get a room template')|
...
- describe how Push It filters model elements by Design option / sets. Note limitations on this.

### ADVANCED - Auditing the model

- describe the Audit process and resulting information.
