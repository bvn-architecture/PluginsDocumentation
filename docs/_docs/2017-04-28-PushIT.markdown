...
- Gotchas: txt file format is tab-delimited; if any tab or newline characters (e.g. multiline text) occur in the excel cell values, this breaks the SOA data format. Ensure that no tab or newline characters exist in the cell values before exporting to txt file.
...
- DataGridView is too slow for large amount of rows. Look at better performing GUI component (WPF?).
...
These custom-shaped Room families are not resizable in any dimension. This is because the shape is arbitrary and hence it cannot be parameterized.
