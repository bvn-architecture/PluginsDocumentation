...
As a first step to using Push It for a Revit model, the SOA data must first be loaded into the UI. Clicking the 'Load SOA...' button in the lower lefthand corner opens up a dialog for browsing to the desired SOA data txt file. (Preparing this SOA txt file is described later on.) Once this file is chosen, Push It loads the data into the UI and checks this against the Revit model data to see what SOA items exist in the model already. As Push It is loading the data, it will check that the item IDs (first column) are unique and that the data in each row is of the appropriate form for the corresponding Revit parameter. It will show an error message if either of these requirements are not met.

ORDER TO COVER THINGS IN (THESE ARE NOT THE HEADINGS):
- Describe the window (meaning of SOA colours, text, columns) NOT THE BUTTONS YET.
- Load.
- How to push.
- Filtering.
- Clear.
- Extra details about pushing (e.g. view with design option / phase required, duplicates, pushing and one comes back, etc.)
- Refresh
- Push Revised
- Audit
