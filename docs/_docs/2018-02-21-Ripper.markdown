...
This app is meant to be run without any other Revit file open. In fact, Revit may crash if a file which is currently open is being detached using Ripper. 
...
| 3 | Source folder path containing the Revit files you want to detach. |
| 4 | Switches between file server (FS) or Revit server (RS) file location. Default is file server i.e. P:\ drive |
| 5 | Folder path to where the detached files will be saved to. This can be the same as the source folder path if files just need to be overwritten. |
...
This dialogue can be used to define which worksets are not opened during the detach process.i.e all worksets known to contain Revit links could be closed to considerable speed up the process. 
...
![RipperMainExport_DWG]({{ site.baseurl }}/assets/Ripper/GUI-Ripper-Export-DWG-doc.png){:class="img-responsive"}{: height="455px" width="385px"}
...
![RipperMainExport_DWFx]({{ site.baseurl }}/assets/Ripper/GUI-Ripper-Export-DWFx-doc.png){:class="img-responsive"}{: height="455px" width="385px"}
